...
function checkAllowance(req, res, proceed) {
    if (req.user instanceof Error) {
        res.status(401)

        return res.json(fail('not allowed', req.user));
    }
...
mainRouter.post('/api/login', [jsonBodyParser, passport.authenticate('local', { session: false }), checkAllowance], handlerLoginUser);
